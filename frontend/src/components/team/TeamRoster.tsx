    ...
    <div className="p-6 bg-surface-card border border-border">
        ...
        {slots.map(({ position, player }) => (
          <div
            ...
            className={`
              ...
              ${player ? 'bg-surface-deep border-border' : 'bg-surface-deep/50 border-border border-dashed'}
            `}
          >
            {/* Position Number */}
            <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center font-broadcast text-lg ${
              player ? 'bg-brand-red text-white' : 'bg-white/5 text-muted-foreground'
            }`}>
              {position}
            </div>
              ...
              <>
                <div className="flex-1 min-w-0">
                  <Link
                    ...
                    className="font-medium text-sm text-white truncate hover:text-accent-amber block"
                ...
                <div className="flex-shrink-0 text-right mr-2">
                  <div className="font-broadcast text-xl text-accent-amber">
                    {player.hrsTotal}
                  </div>
                ...
                <Button
                  ...
                  className="flex-shrink-0 h-8 w-8 p-0 text-muted-foreground hover:text-red-400 hover:bg-red-500/10"
                >
                  <X className="h-4 w-4" />
                </Button>
              </>
      ...
      <div className="border-t border-border pt-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            {totalHRs} <span className="text-muted-foreground">/ {maxHRs}</span>
          </span>
        </div>

        {/* Validation Messages */}
        {needsMorePlayers && (
          <div className="text-sm text-accent-amber flex items-center gap-2">
